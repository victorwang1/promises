...
const lib = require('../../lib/advancedChainingLib.js');
...
var searchCommonTagsFromGitHubProfiles = function(githubHandles) {
  return Promise.all(githubHandles.map(handle => lib.getGitHubProfile(handle)))
                .then(profiles => Promise.all(profiles.map(profile => lib.tagImage(profile.avatarUrl))))
                .then(tags => lib.getIntersection(tags))
